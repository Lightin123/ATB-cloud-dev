...
const EditSettings = () => {
  ...
  const [updateSettings, { isLoading, isError, error, isSuccess }] =
    useUpdateUserMutation();
  ...
  const deleteConfirmation = settingsForm.watch("deleteConfirmation");
    ...
    <div className="mt-2 w-[100%]">
          ...
          {themes.map((theme, index) => {
            if (theme.value === "system") {
              return (
                <div key={index} className="flex flex-col gap-2">
                  <div
                    className={cn(
                      "w-16 h-16 cursor-pointer flex outline-4 outline-foreground hover:outline rounded-lg",
                      currentTheme === theme.value && " outline rounded-lg "
                    )}
                    onClick={() => setTheme(theme.value)}
                  >
                    <div className="h-full w-full bg-gray-50 rounded-l-lg p-1 border-2 border-gray-50 border-r-0">
                      <p className="font-500 text-lg text-black">Aa</p>
                    </div>
                    <div className="h-full w-full bg-gray-800 rounded-r-lg p-1 border-2 border-black border-l-0">
                      <p className="font-500 text-lg text-white">Aa</p>
                    </div>
                  </div>
                  <p className="text-foreground font-500">{theme.label}</p>
                </div>
              );
            }
            ...
            return (
              <div key={index} className="flex flex-col gap-2">
                <div
                  className={cn(
                    "p-2 cursor-pointer rounded-lg w-16 h-16 border-2 outline-4 outline-foreground hover:outline",
                    theme.value === "light" &&
                      "bg-gray-50 border-gray-100 text-black",
                    theme.value === "dark" &&
                      "bg-gray-900 text-white border-gray-800",
                    currentTheme === theme.value && " outline"
                  )}
                  onClick={() => setTheme(theme.value)}
                >
                  <p className="font-500 text-lg">Aa</p>
                </div>
                <p className="text-foreground font-500">{theme.label}</p>
              </div>
            );
      ...
      <Form {...settingsForm}>
        <form
          ...
          className="flex flex-col gap-y-3 w-[100%] "
        >
          <FormField
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => deleteUserById(userData.id)}
            >
